...
const Home = () => {
    ...
    const [filter, setFilter] = useState({ category: "", price: "", brand: "", rating: "" });
    ...
    const filteredProducts = products.filter((p) => {
        return (
            ...
            (!filter.price ||
                (filter.price === "low" && p.price < 75) ||
                (filter.price === "high" && p.price >= 75)) &&
            (!filter.rating ||
                (filter.rating === "low" && p.rating < 3) ||
                (filter.rating === "high" && p.rating >= 5))
        );
    });
    ...
    return (
        <>
            ...
            {/* <div className="my-4 mx-auto flex w-10/12 flex-col items-start justify-between">
                                <div className="mb-2 flex">
                                    <p className="mr-3 text-sm font-semibold">{data?.price}</p> */}
            {/* <del className="text-xs text-gray-400"> $79.00 </del> */}
            {/* </div>
                                <h3 className="mb-2 text-sm text-gray-400">{data?.title}</h3>
                            </div> */}
        ...
        {['', 'men', 'women', 'electronics','jewelery'].map((cat) => (
          <button key={cat} className="border px-4 py-2 rounded" onClick={() => setFilter({ ...filter, category: cat })}>
            {cat || 'All'}
          </button>
        ))}
      </div>
          ...
          <option value="high">$75 & Above</option>
                ...
                {!selectedProduct ? (
                        ...
                        {filteredProducts.map((product) => (
                            <div key={product.id} className="border p-4 rounded shadow flex flex-col h-full">
                                ...
                                <div className="flex-grow flex flex-col">
                                    ...
                                    <button
                                        className="mt-auto w-full py-2 text-blue-500 hover:bg-blue-50 rounded transition-colors"
                    ...
                    <div className="p-4 bg-white rounded shadow space-y-4">
                        <button className="text-sm text-blue-500 underline" onClick={() => setSelectedProduct(null)}>← Back to Products</button>
                        <img src={selectedProduct.image} alt={selectedProduct.name} className="w-60 rounded" />
                        <h2 className="text-2xl font-semibold">{selectedProduct.name}</h2>
                        <p>{selectedProduct.description}</p>
                        <p className="text-xl font-bold">${selectedProduct.price}</p>
                        <button className="px-4 py-2 bg-blue-600 text-white rounded" onClick={() => addToCart(selectedProduct)}>Add to Cart</button>
                    </div>
                )}
                ...
                <div className="p-4 bg-gray-100 rounded-lg">
                    ...
                    {cart.length > 0 && (
                        <button className="mt-4 px-4 py-2 bg-green-600 text-white rounded" onClick={() => setCheckout(true)}>Proceed to Checkout</button>
                    )}
                </div>
                ...
                {checkout && (
                    <div className="p-4 bg-white rounded-lg shadow-md">
                        ...
                        <div className="space-y-3">
                            ...
                            <button className="px-4 py-2 bg-blue-600 text-white rounded" onClick={handleCheckout}>Submit Order</button>
                        </div>
                    </div>
                )}
            </div>
